RSpec.shared_examples "v2x1_test_tags_subcollection" do |primary_collection|
  ...
  describe("tags_subcollections") do
    describe("/api/v2.1/#{primary_collection}/:id/tags") do
      let(:tags_subcollection) { "/api/v2.1/#{primary_collection}/#{instance.id}/tags" }
      ...
      context "tag" do
        let(:instance_tag_url) { "/api/v2.1/#{primary_collection}/#{instance.id}/tag" }
        ...
        it "not yet tagged" do
          ...
          expect(response).to have_attributes(
            :status => 201,
            :location => "http://www.example.com/api/v2.1/#{primary_collection}/#{instance.id}/tags",
            :parsed_body => payload
          )
        end
      ...
      context "untag" do
        it "works" do
          ...
          post("/api/v2.1/#{primary_collection}/#{instance.id}/untag", :params => payload.to_json, :headers => headers)
