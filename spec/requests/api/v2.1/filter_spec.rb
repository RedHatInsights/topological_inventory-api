RSpec.describe("::Insights::API::Common::Filter") do
  ...
  def expect_failure(query, *errors)
    get(URI.escape("/api/v2.1/tasks?#{query}"), :headers => headers)
  ...
  context "sorted results via sort_by" do
    ...
    it "available for vms with default order" do
      get("/api/v2.1/vms?filter[name][starts_with]=sort_by_vm&sort_by=name", :headers => headers)
    ...
    it "available for vms with desc order" do
      get("/api/v2.1/vms?filter[name][starts_with]=sort_by_vm&sort_by=name:desc", :headers => headers)
  ...
  context "error cases" do
    ...
    it "unsupported attribute type" do
      ...
      get("/api/v2.1/vms?filter[mac_addresses]=a", :headers => headers)
    ...
    it "invalid attribute" do
      ...
      get("/api/v2.1/vms?filter[bogus_attribute]=a", :headers => headers)
    ...
    it "multiple invalid attributes mixed with a valid attribute" do
      ...
      get("/api/v2.1/vms?filter[mac_addresses]=a&filter[name]=a&filter[bogus_attribute]=b", :headers => headers)
