RSpec.describe SourcesController, type: :controller do
  it "delete /sources/:id deletes a Source" do
    ...
    delete_path(source_url(source.id))
  ...
  it "patch /sources/:id updates a Source" do
    ...
    patch_path(source_url(source.id), :params => {:name => "xyz"})
  ...
  it "post /sources creates a Source" do
    post_path(sources_url, :body => {:name => "abc"}.to_json, :format => :json)

    source = Source.first

    expect(response.status).to eq(201)
    expect(response.location).to match(a_string_ending_with("/sources/#{source.id}"))
