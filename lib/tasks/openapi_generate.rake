class OpenapiGenerator
  ...
  def graphql_add_querytype(sfd, collections)
    sfd.puts("")
  ...
  def graphql_generate(openapi_content)
    ...
    resources.each do |resource|
      ...
      this_schema = schemas[klass_name]
      ...
      properties.keys.sort.each do |property_name|
        ...
        if property_schema["$ref"]
          ref_schema = property_schema["$ref"].match("^##{SCHEMAS_PATH}/([^/]*)$")
          property_schema = schemas[ref_schema[1]]
        end
  ...
  def run(graphql)
    ...
    graphql_generate(new_content) if graphql
  end
end
